...
const setUpBotBtn = () => {
   ...
   codeForBotBtn.addEventListener('click', () => {
      ...
      const firstSkinId = document.querySelectorAll('input.price-input')[0].id;
      const secondSkinId = document.querySelectorAll('input.price-input')[9].id;
      const hrefs = [];
      const amounts = [];
      const maxFloat = [];
      const maxPrice = [];
      let firstAmount = 0, secondAmount = 0;
      const firstPrice = Number(document.querySelectorAll('input.price-input')[0].value)
      const secondPrice = Number(document.querySelectorAll('input.price-input')[9].value)
      ...
      for (let item of checkboxes) {
         if (item && item.checked == true) {
            ...
            if (firstSkinId.indexOf(replacementId) != -1) firstAmount = amount;
